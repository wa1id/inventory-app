 ...
 *
 * Must stay byte-compatible with the app's `src/services/account/base32.ts`.
 * The two files are one unit: a change here that is not mirrored there strands
 * every existing backup behind a code that no longer decodes.
 */
 ...
 *
 * Null rather than a throw because every caller here is handling untrusted
 * input — a request header or a text field someone is still typing into.
 */
